...
% Generate ROC curve - Based on sample code (LDAwithROCcurve.m)
[Pfp, Ptp, Pfn, Perror, thresholdList] = ROCcurve(discriminantScore, labelsVal, [P_L0, P_L1]);

% Find empirically optimal gamma 
...
% Visualize logistic-linear results - Adapted from discussion with Claude
for idx = 1:3
    xTrain = datasets{idx};
    labelsTrain = labels_datasets{idx};
...
% Visualize logistic-quadratic results - Adapted from discussion with Claude
figure(6), clf;
for idx = 1:3
    xTrain = datasets{idx};
    labelsTrain = labels_datasets{idx};
...
% Summary plot - Adapted from discussion with Chatgpt
figure(7), clf;
...
fprintf('4. Quadratic model shows overfitting at N=50 but improves with more data\n\n');


% HELPER FUNCTIONS

% Code for evalGaussian function taken from sample code (evalGaussian.m)
...
% Code for generateDataFromGMM function taken from sample code (generateDataFromGMM.m)
...
% Code for ROCcurve function adapted from sample code (LDAwithROCcurve.m)
